...
    
    let response;
    if (messageText.toLowerCase().includes('good morning') && messageText.toLowerCase().includes('glucose')) {
      response = "Good Morning! You are trending exactly as we planned so I'm feeling good about the metabolic system regulation and balance. This will serve you well as you face today's challenge head on. Remember...\"The ocean doesn't stop being the ocean because a storm passes through.\" Now, let's start with some breathing to calibrate and reset your central nervous system. Then we will develop a plan of attack together to make today a major win for you.";
    } else {
      const responses = [
        "Excellent question, Boss. Based on your patterns over the past few months, I'm seeing some interesting connections to your energy cycles. Let me ask you this - how has your sleep quality been affecting your decision-making clarity?",
        "That's a fascinating perspective. From a biometric standpoint, I'm curious about the correlation between your HRV readings and these decision points. Have you noticed any patterns in your glucose levels during high-stress periods?",
        "I appreciate you sharing that insight. Your body is constantly giving you data - let's decode what it's telling us. Based on your recent biomarkers, I'd suggest we optimize your circadian rhythm first.",
        "Boss, this is where the magic happens. Your question touches on something I've been tracking in your data patterns. The correlation between your cortisol rhythms and decision fatigue is particularly interesting.",
        "This reminds me of our discussion about cognitive load management from last month. Given your current stress markers and recovery metrics, I think we should explore this from both a tactical and strategic angle."
      ];
...
          <span>✨</span>
          <span>Welcome to the future of DI interaction</span>
        </div>
 
...
              <div>
                <div 
                  className="text-lg font-normal mb-1 bg-gradient-to-r from-green-400 via-cyan-400 to-purple-500 bg-clip-text text-transparent cursor-pointer hover:opacity-80 transition-opacity"
                  onClick={() => {
...
 
            <div className="mb-8">
              <div className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-4 px-1">YOUR LIFEPRINT™</div>
              <div className="space-y-2">
                <button 
                  onClick={() => {
...
                    setSidebarOpen(false);
                  }}
                  className="w-full p-3 rounded-xl flex items-center gap-3 text-gray-400 hover:text-white hover:bg-white/5 transition-all text-sm"
                >
...
              <div className="bg-white/10 backdrop-blur-lg px-4 py-3 rounded-2xl rounded-bl-md border border-white/10">
...
        </div>
 
        <div className="p-4 bg-black/20 backdrop-blur-xl border-t border-white/10 relative z-30">
          <div className="flex items-center gap-3">
            <button className="w-9 h-9 rounded-full bg-gray-700/70 flex items-center justify-center text-gray-300 hover:text-white hover:bg-gray-600/70 transition-all">
...
                {!inputText.trim() && (
                  <>
                    <button className="w-8 h-8 rounded-full bg-gray-600/50 flex items-center justify-center text-gray-300 hover:text-white hover:bg-gray-500/50 transition-all">
                      <Mic className="w-4 h-4" />
                    </button>
                    
                    <button 
                      onClick={() => setCurrentScreen('voice-interface')}
                      className="w-8 h-8 rounded-full bg-gray-600/50 flex items-center justify-center text-gray-300 hover:text-white hover:bg-gray-500/50 transition-all"
...
 
    const getStatusText = () => {
      return voiceState === 'listening' ? 'Listening...' : 'Dr. Kai is responding...';
    };
 
    const getAdvisorStatus = () => {
      return voiceState === 'listening' ? 'Listening' : 'Sharing';
    };
 
    useEffect(() => {
...
      <>
        <style>{`
          /* No custom CSS needed - keeping it clean */
        `}</style>
        
        <div className="h-screen flex flex-col bg-gradient-to-br from-black via-gray-900 to-black relative">
          <div className="flex items-center justify-between p-4 bg-black/20 backdrop-blur-xl border-b border-white/10 relative z-30">
...
 
            <div className="flex items-center gap-2 text-sm text-gray-400">
              <div className={`w-2 h-2 rounded-full bg-gradient-to-r transition-all duration-500 ${
                modes[selectedMode].emoji === '⚡' ? 'from-yellow-400 to-orange-400' :
                modes[selectedMode].emoji === '⚖️' ? 'from-green-400 to-emerald-400' :
                modes[selectedMode].emoji === '🌱' ? 'from-purple-500 to-purple-400' :
                'from-blue-400 to-cyan-400'
...
                <div className={`w-64 h-64 rounded-full bg-gradient-to-r ${advisor.gradient} flex items-center justify-center text-6xl breathing-avatar-ready`}>
                  <div className="w-56 h-56 rounded-full bg-gray-900 flex items-center justify-center text-5xl">
                    {advisor.emoji}
                  </div>
                </div>
              </div>
...
                }`}></div>
                <span className="text-gray-300">{getAdvisorStatus()}</span>
              </div>
            </div>
 
            <div className="mb-20">
...
 
            <div className="text-center">
              <p className="text-2xl font-light text-white">{getStatusText()}</p>
            </div>
          </div>
 
...
              className="w-14 h-14 rounded-full bg-gray-700/70 backdrop-blur-sm flex items-center justify-center text-gray-400 hover:text-white hover:bg-gray-600/70 transition-all duration-300 hover:scale-105"
            >
              <X className="w-6 h-6" />
            </button>
          </div>
...
                    <p className="text-gray-400 text-sm">Awesome!...</p>
...
                      }
                    }}
                    className="w-full p-3 rounded-xl flex items-center gap-3 transition-all text-sm text-gray-300 hover:text-white hover:bg-white/5"
                  >
                    <div className="w-6 h-6 rounded-full bg-cyan-400 flex items-center justify-center text-xs">
                      {adv.emoji}
                    </div>
                    <div className="text-left">
                      <div className="font-medium">{adv.name}</div>
...
              <div className="space-y-2">
                <button 
                  className="w-full p-3 rounded-xl flex items-center gap-3 text-white bg-amber-500/30 transition-all text-sm"
                >
                  <div className="w-6 h-6 rounded-full bg-amber-500/40 flex items-center justify-center text-xs">📚</div>
                  <span>Codex</span>
                </button>
                <button className="w-full p-3 rounded-xl flex items-center gap-3 text-gray-400 hover:text-white hover:bg-white/5 transition-all text-sm">
                  <div className="w-6 h-6 rounded-full bg-blue-500/20 flex items-center justify-center text-xs">📊</div>
...
              Council Codex
            </div>
            <div className="text-sm text-gray-400">Your Personal Playbook</div>
          </div>
 
          <div className="w-10"></div>
        </div>
...
          <div className="max-w-4xl mx-auto">
            <h2 className="text-3xl font-semibold mb-6">📚 Council Codex</h2>
            <p className="text-gray-400 mb-8">Your personal playbook is being built!</p>
            
...
                <p className="text-gray-400 text-sm">Wind down and reflect</p>
              </div>
              
              <div className="bg-white/5 backdrop-blur-lg rounded-xl p-6 border border-white/10">
                <div className="text-3xl mb-4">🧪</div>
                <h3 className="text-lg font-semibold mb-2">Stacks</h3>
                <p className="text-gray-400 text-sm">Supplements and compounds</p>
              </div>
              
              <div className="bg-white/5 backdrop-blur-lg rounded-xl p-6 border border-white/10">
                <div className="text-3xl mb-4">🍽️</div>
                <h3 className="text-lg font-semibold mb-2">Meals</h3>
                <p className="text-gray-400 text-sm">Nutrition protocols</p>
              </div>
              
              <div className="bg-white/5 backdrop-blur-lg rounded-xl p-6 border border-white/10">
                <div className="text-3xl mb-4">💪</div>
                <h3 className="text-lg font-semibold mb-2">Exercise</h3>
                <p className="text-gray-400 text-sm">Movement and training</p>
              </div>
              
              <div className="bg-white/5 backdrop-blur-lg rounded-xl p-6 border border-white/10">
                <div className="text-3xl mb-4">📖</div>
                <h3 className="text-lg font-semibold mb-2">Doctrine</h3>
                <p className="text-gray-400 text-sm">Core principles and wisdom</p>
              </div>
            </div>
          </div>
        </div>
