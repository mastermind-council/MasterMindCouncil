...
const MasterMindCouncil = () => {
  ...
  useEffect(() => {
    if (currentScreen !== 'text-interface') {
      // Don't clear messages when coming from archive with a loaded conversation
  ...
  const handlePasswordToggle = () => {
    ...
    // Restore both values after re-render
    setTimeout(() => {
      if (emailRef.current) {
  ...
  const handleLogin = () => {
    ...
    // Only advance if both fields have content
    if (email && password) {
  ...
  const handleSendMessage = (inputElement) => {
    ...
    // Demo conversation - if user types the morning message, respond specifically
    let response;
    if (messageText.toLowerCase().includes('good morning') && messageText.toLowerCase().includes('glucose')) {
      ...
      // Other authentic Dr. Kai responses
      const responses = [
        ...
        <div className={`fixed left-0 top-0 h-full w-80 bg-black/90 backdrop-blur-xl border-r border-white/10 z-50 transform transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          <div className="p-6">
            ...
            <div className="flex items-center justify-between mb-8">
              <div>
                <div className="text-lg font-normal mb-1 bg-gradient-to-r from-green-400 via-cyan-400 to-purple-500 bg-clip-text text-transparent">
                  THE MASTER MIND COUNCIL™
                </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 flex items-center gap-3 bg-white/10 rounded-full px-4 py-3 border border-white/20 focus-within:border-purple-500/50 input-container">
              ...
              <div className="flex items-center gap-2">
                {/* Microphone and Waveform - only show when input is empty */}
                {!inputText.trim() && (
                  <>
                ...
                {/* Send button - always visible for now */}
                <button
                  onClick={(e) => {
  ...
  const VoiceInterface = () => {
    const advisor = advisors[selectedAdvisor];
    const [voiceState, setVoiceState] = useState('ready'); // 'ready', 'listening', 'responding'
    ...
    const handleVoiceClick = () => {
      if (voiceState === 'ready') {
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            setVoiceState('ready');
          }, 3000); // Dr. Kai speaks for 3 seconds
        }, 2000); // User speaks for 2 seconds
      }
    };
    ...
    return (
      <div className="h-screen flex flex-col bg-gradient-to-br from-black via-gray-900 to-black relative">
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-8 relative z-10">
          {/* Dr. Kai Avatar and Status */}
          <div className="text-center mb-16">
            <div className="relative inline-block mb-4">
              <div className={`w-20 h-20 rounded-full bg-gradient-to-r ${advisor.gradient} flex items-center justify-center text-3xl relative ${voiceState === 'responding' ? 'animate-pulse' : ''}`}>
                <div className={`absolute inset-0 rounded-full bg-gradient-to-r ${advisor.gradient} ${voiceState === 'listening' ? 'animate-ping' : ''}`}></div>
            ...
            <div className="flex items-center justify-center gap-2 text-sm">
              <div className={`w-2 h-2 rounded-full ${
                voiceState === 'listening' ? 'bg-blue-400 animate-pulse' :
                voiceState === 'responding' ? 'bg-green-400 animate-pulse' :
                'bg-gray-400'
              }`}></div>
            ...
            <button
              ...
              className={`relative w-32 h-32 rounded-full flex items-center justify-center transition-all duration-300 ${
                voiceState === 'listening'
                  ...
                  : 'bg-cyan-500 hover:bg-cyan-600 shadow-lg shadow-cyan-500/30 hover:scale-105'
              } ${voiceState !== 'ready' ? 'cursor-not-allowed' : 'cursor-pointer'}`}
            >
              {/* Ripple effect for listening state */}
              {voiceState === 'listening' && (
                <>
                  <div className="absolute inset-0 rounded-full bg-blue-400 animate-ping opacity-30"></div>
                  <div className="absolute inset-2 rounded-full bg-blue-300 animate-ping opacity-20" style={{ animationDelay: '0.5s' }}></div>
                </>
              )}
              
              <Mic className={`w-8 h-8 text-white relative z-10 ${voiceState === 'listening' ? 'animate-pulse' : ''}`} />
            </button>
          </div>
          ...
          <div className="text-center">
            ...
            <p className="text-sm text-gray-400">
              {voiceState === 'ready' ? 'Tap to start conversation' : ''}
            </p>
          </div>
        </div>
  ...
  const DrKaiArchiveScreen = () => {
    // Handle loading specific conversation
